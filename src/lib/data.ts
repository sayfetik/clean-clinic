...
export const infusionCatalog = {
  ...
  filters: ['Все капельницы', 'Похудение', 'Омоложение', 'Поддержка здоровья', 'Насыщение организма']
}

// ендпоинты для изменения и получения страницы о клинике, пока так:
export const about = {
  title: 'Инфузионная терапия',
  text1: 'Капельная или инфузионная терапия — метод лечения, основанный на прямом введении в кровоток различных коктейлей медикаментов, витаминов и микроэлементов, с целью быстрого достижения лучших показателей для вашего здоровья.',
  text2: 'IV-терапия только лечит различные заболевания но также используется для профилактики и общего оздоровления организма. Она помогает нормализовать водно-солевой обмен, очищать организм от токсинов и укреплять иммунитет. Индивидуальный подход к составлению капельниц позволяет каждому пациенту получить максимальную пользу от лечения.',
  specialists: [
